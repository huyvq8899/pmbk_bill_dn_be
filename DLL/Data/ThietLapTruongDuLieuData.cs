...
namespace DLL.Data
{
    public class ThietLapTruongDuLieuData
    {
        ...
        public List<ThietLapTruongDuLieu> InitData()
        {
            ...
            List<ThietLapTruongDuLieu> data = new List<ThietLapTruongDuLieu>
            {
                ...
                new ThietLapTruongDuLieu
                {
                    ThietLapTruongDuLieuId = Guid.NewGuid().ToString(),
                    MaTruong = null,
                    TenCot = nameof(hoaDonDienTu.PosCustomerURL),
                    TenTruong = "Máy POS",
                    TenTruongHienThi = "Máy POS",
                    LoaiHoaDon = LoaiHoaDon.None,
                    LoaiTruongDuLieu = LoaiTruongDuLieu.NhomBangKe,
                    KieuDuLieu = KieuDuLieuThietLapTuyChinh.Chu,
                    GhiChu = null,
        ...
        public string QueryInsertPosCustomerURL()
        {
            var hoaDonDienTu = new HoaDonDienTu();
            const string PosCustomerURL = nameof(hoaDonDienTu.PosCustomerURL);

            var posCustomerURL = InitData()
                .Where(x => x.LoaiTruongDuLieu == LoaiTruongDuLieu.NhomBangKe && x.LoaiHoaDon == LoaiHoaDon.None && x.TenCot == PosCustomerURL)
                .ToList();

            var result = $"DELETE FROM ThietLapTruongDuLieus WHERE TenCot = '{PosCustomerURL}'";
            result += Query(posCustomerURL);
            return result;
        }
