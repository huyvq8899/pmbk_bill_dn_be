...
namespace BKSOFT.SOCKET
{
    public class Server : IDisposable
    {
            ...
            while (_isRunning)
            {
                try
                {
                    ...
                    if (socket != null)
                    {
                        ...
                        client.ClientConvertionToPDF += OnClientConvertionToPDF;
                        client.Start();
        ...
        private void OnClientConvertionToPDF(string result)
        {
            try
            {
                if(!string.IsNullOrEmpty(result) && result == "true")
                {
                    this.NumConvertSuccess += 1;
                }    
                else if(!string.IsNullOrEmpty(result) && result == "false")
                {
                    this.NumConvertError += 1;
                }    
            }
            catch (Exception ex)
            {
