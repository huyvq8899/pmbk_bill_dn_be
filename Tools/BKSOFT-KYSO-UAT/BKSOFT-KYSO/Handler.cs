        ...
        public static string ProcessData(string encode)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // Fix for 3 MST TVAN TEST
                    if (msg.MST.Contains("0107726588"))
                    {
                        string pfxFilePath = Path.Combine(path, "SDS_TVAN/540110000AC102860AF96FE19C387D55_0107726588.p12");
                        cert = new X509Certificate2(pfxFilePath, "1");
                    }
                    else if (msg.MST.Contains("8140639400"))
                    {
                        string pfxFilePath = Path.Combine(path, "SDS_TVAN/540110005BA440EE15A6FB40498D1A27_8140639400.p12");
                        cert = new X509Certificate2(pfxFilePath, "1");
                    }
                    else if (msg.MST.Contains("0107726098"))
                    {
                        string pfxFilePath = Path.Combine(path, "SDS_TVAN/5401100067467F50BB0B7FB4A5C73C58_0107726098.p12");
                        cert = new X509Certificate2(pfxFilePath, "1");
                    }    
                    else
                    {
