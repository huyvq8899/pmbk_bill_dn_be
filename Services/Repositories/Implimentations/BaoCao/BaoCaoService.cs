        ...
        private async Task<List<BangKeHangHoaBanRaViewModel>> GetBangKeHangHoaBanRaAsync(HoaDonParams @params)
        {
            ...
            try
            {
                ...
                var query = from hd in _db.HoaDonDienTus
                            ...
                            && (hd.TrangThaiQuyTrinh == (int)TrangThaiQuyTrinh.CQTDaCapMa || hd.TrangThaiQuyTrinh == (int)TrangThaiQuyTrinh.GuiKhongLoi || hd.TrangThaiQuyTrinh == (int)TrangThaiQuyTrinh.DaKyDienTu || hd.TrangThaiQuyTrinh == (int)TrangThaiQuyTrinh.HoaDonHopLe)
                            where @params.LoaiNghiepVu == 1 ? (hd.LoaiHoaDon == 1 || hd.LoaiHoaDon == 2) : @params.LoaiNghiepVu == 3 ? (hd.LoaiHoaDon == 9 || hd.LoaiHoaDon == 10 || hd.LoaiHoaDon == 13 || hd.LoaiHoaDon == 14 || hd.LoaiHoaDon == 15) : (hd.LoaiHoaDon == 7 || hd.LoaiHoaDon == 8)
                            ...
                            select new BangKeHangHoaBanRaViewModel
                            {
                                ...
                                TenKhachHang = string.IsNullOrEmpty(g.Key.hd.TenKhachHang) ? g.Key.hd.HoTenNguoiMuaHang : g.Key.hd.TenKhachHang,
                                MaSoThue = g.Key.hd.MaSoThue,
                                ThueSuat = g.Key.ThueGTGT,
                                TrangThai = g.Key.hd.TrangThai.Value,
                                HoaDonDienTuId = g.Key.hd.HoaDonDienTuId,
                                TongTienChuaThue = g.Key.hd.TrangThai == (int)TrangThaiHoaDon.HoaDonXoaBo ? 0 : g.Sum(e => g.Key.hd.MaLoaiTien == "VND" ? e.ThanhTien : e.ThanhTienQuyDoi).Value,
                                TongTienThueGTGT = g.Key.hd.TrangThai == (int)TrangThaiHoaDon.HoaDonXoaBo ? 0 : g.Sum(e => g.Key.hd.MaLoaiTien == "VND" ? e.TienThueGTGT : e.TienThueGTGTQuyDoi).Value,
