                ...
                foreach (var item in companyModels)
                {
                    using (SqlConnection connection = new SqlConnection(item.ConnectionString))
                    {
                        string query = $@"SELECT COUNT(*) FROM HoaDonDienTus hd
                                        ...
                                        WHERE hd.SoHoaDon = @SoHoaDon AND kh.KyHieuHoaDon = @KyHieu AND kh.KyHieuMauSoHoaDon = @MauSo";
                        using (SqlCommand command = new SqlCommand(query, connection))
                        {
                            command.Parameters.Add("@SoHoaDon", SqlDbType.BigInt);
                            command.Parameters["@SoHoaDon"].Value = input.SoHoaDon;
                            command.Parameters.Add("@KyHieu", SqlDbType.NVarChar);
                            command.Parameters["@KyHieu"].Value = input.KyHieuHoaDon;
                            command.Parameters.Add("@MauSo", SqlDbType.Int);
                            command.Parameters["@MauSo"].Value = input.KyHieuMauSoHoaDon;
