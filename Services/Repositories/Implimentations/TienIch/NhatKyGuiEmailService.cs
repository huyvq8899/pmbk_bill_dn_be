...
namespace Services.Repositories.Implimentations.TienIch
{
    public class NhatKyGuiEmailService : INhatKyGuiEmailService
    {
        ...
        public async Task<PagedList<NhatKyGuiEmailViewModel>> GetAllPagingAsync(NhatKyGuiEmailParams @params)
        {
            var query = from nk in _db.NhatKyGuiEmails
                        ...
                        select new NhatKyGuiEmailViewModel
                        {
                            ...
                            IsCapSo = string.IsNullOrEmpty(soHoaDon) ? false : true,
                            SoHoaDon = string.IsNullOrEmpty(nk.So) ? (long?)null : long.Parse(nk.So),
                            Ngay = nk.Ngay,
            ...
            if(@params.LoaiHoaDon != -1)
            {
                if(@params.LoaiHoaDon == 1)
                {
                    query = query.Where(x => x.LoaiEmail == LoaiEmail.ThongBaoPhatHanhHoaDon
                    || x.LoaiEmail == LoaiEmail.ThongBaoXoaBoHoaDon
                    || x.LoaiEmail == LoaiEmail.ThongBaoBienBanHuyBoHoaDon
                    || x.LoaiEmail == LoaiEmail.ThongBaoBienBanDieuChinhHoaDon
                    || x.LoaiEmail == LoaiEmail.ThongBaoSaiThongTinKhongPhaiLapLaiHoaDon
                    || x.LoaiEmail == LoaiEmail.ThongBaoSaiThongTinKhongPhaiLapLaiHoaDonHinhThucCoMaTuMTT);
                }
                else if(@params.LoaiHoaDon == 2)
                {
                    query = query.Where(x => x.LoaiEmail == LoaiEmail.ThongBaoPhatHanhPXK
                    || x.LoaiEmail == LoaiEmail.ThongBaoXoaBoPXK
                    || x.LoaiEmail == LoaiEmail.ThongBaoBienBanHuyBoPXK
                    || x.LoaiEmail == LoaiEmail.ThongBaoBienBanDieuChinhPXK
                    || x.LoaiEmail == LoaiEmail.ThongBaoSaiThongTinKhongPhaiLapLaiPXK);
                }
            }
            ...
            if (!string.IsNullOrEmpty(@params.SortKey))
            {
                ...
                if (@params.SortKey == nameof(@params.Filter.So))
                {
                    if (@params.SortValue == "ascend")
                    {
                        query = query.OrderBy(x => x.IsCapSo).ThenBy(x => x.SoHoaDon ?? 0);
                    }
                    if (@params.SortValue == "descend")
                    { 
                        query = query.OrderBy(x => x.IsCapSo).OrderByDescending(x => x.SoHoaDon ?? 0);
                    }
                }
                if (@params.SortKey == nameof(@params.Filter.StrKyHieu))
                {
        ...
        public async Task<NhatKyGuiEmailViewModel> GetNhatKyGuiEmailByHoaDonDienTuIdAsync(string hoaDonDienTuId)
        {
            var query = _db.NhatKyGuiEmails.Where(x => x.NhatKyGuiEmailId == hoaDonDienTuId  && (x.TrangThaiGuiEmail == TrangThaiGuiEmail.DaGui || x.TrangThaiGuiEmail == TrangThaiGuiEmail.KhachHangDaNhan)).OrderByDescending(x => x.ModifyDate)
                .Select(nk => new NhatKyGuiEmailViewModel
                {
                    ...
                    thongBaoSaiThongTin = _db.ThongBaoSaiThongTins.Where(t => t.NhatKyGuiEmailId == nk.NhatKyGuiEmailId).Select(tt => new ThongBaoSaiThongTin
                    {
                        ...
                        DiaChi_Dung = tt.DiaChi_Dung,
                        DiaChi_Sai = tt.DiaChi_Sai,
                        ThongTinGiaiTrinhSaiSot = tt.ThongTinGiaiTrinhSaiSot,
                        EmailCuaNguoiNhan = tt.EmailCuaNguoiNhan,
                        EmailBCCCuaNguoiNhan = tt.EmailBCCCuaNguoiNhan,
                        EmailCCCuaNguoiNhan = tt.EmailCCCuaNguoiNhan,
                        TenNguoiNhan = tt.TenNguoiNhan,
