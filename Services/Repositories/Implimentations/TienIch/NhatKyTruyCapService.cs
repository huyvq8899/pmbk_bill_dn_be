    ...
    public class NhatKyTruyCapService : INhatKyTruyCapService
    {
        ...
        public async Task<PagedList<NhatKyTruyCapViewModel>> GetAllPagingAsync(NhatKyTruyCapParams @params)
        {
            ...
            if (!string.IsNullOrEmpty(@params.SortKey))
            {
                ...
                //    }
                //}
                if (@params.SortKey == "CreatedDateFilter")
                {
                    if (@params.SortValue == "ascend")
                    {
                ...
                if (@params.SortKey == "CreatedByUserName")
                {
                    if (@params.SortValue == "ascend")
                    {
                ...
                if (@params.SortKey == "DoiTuongThaoTac")
                {
                    if (@params.SortValue == "ascend")
                    {
                        query = query.OrderBy(x => x.DoiTuongThaoTac);
                    }
                    if (@params.SortValue == "descend")
                    {
                        query = query.OrderByDescending(x => x.DoiTuongThaoTac);
                    }
                } 

                if (@params.SortKey == "HanhDong")
                {
                    if (@params.SortValue == "ascend")
                    {
                        query = query.OrderBy(x => x.HanhDong);
                    }
                    if (@params.SortValue == "descend")
                    {
                        query = query.OrderByDescending(x => x.HanhDong);
                    }
                }
                if (@params.SortKey == "ThamChieu")
                {
                    if (@params.SortValue == "ascend")
                    {
                        query = query.OrderBy(x => x.ThamChieu);
                    }
                    if (@params.SortValue == "descend")
                    {
                        query = query.OrderByDescending(x => x.ThamChieu);
                    }
                }
                
                if (@params.SortKey == "MoTaChiTiet")
                {
                    if (@params.SortValue == "ascend")
                    {
                        query = query.OrderBy(x => x.MoTaChiTiet);
                    }
                    if (@params.SortValue == "descend")
                    {
                        query = query.OrderByDescending(x => x.MoTaChiTiet);
                    }
                }
                if (@params.SortKey == "DiaChiIP")
                {
                    if (@params.SortValue == "ascend")
                    {
                        query = query.OrderBy(x => x.DiaChiIP);
                    }
                    if (@params.SortValue == "descend")
                    {
                        query = query.OrderByDescending(x => x.DiaChiIP);
                    }
                }
        ...
        public async Task<List<NhatKyTruyCapViewModel>> GetByRefIdAsync(string id)
        {
            ...
            var query = from nktc in _db.NhatKyTruyCaps
                        join u in _db.Users on nktc.CreatedBy equals u.UserId
                        where nktc.RefId.Contains(id) || (nktc.RefId == bbHuyId && bbHuyId != null) || (nktc.RefId == bbdcId && bbdcId != null)
                        orderby nktc.CreatedDate descending
                        select new NhatKyTruyCapViewModel
                        {
                            NhatKyTruyCapId = nktc.NhatKyTruyCapId,
