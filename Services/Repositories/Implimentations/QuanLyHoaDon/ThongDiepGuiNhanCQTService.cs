...
using DLieu = Services.ViewModels.XML.ThongDiepGuiNhanCQT.DLieu;
using DLTBao = Services.ViewModels.XML.ThongDiepGuiNhanCQT.DLTBao;
using Document = Spire.Doc.Document;
using DSCKS = Services.ViewModels.XML.ThongDiepGuiNhanCQT.DSCKS;
using HDon = Services.ViewModels.XML.ThongDiepGuiNhanCQT.HDon;
using TBao = Services.ViewModels.XML.ThongDiepGuiNhanCQT.TBao;
using TDiep = Services.ViewModels.XML.ThongDiepGuiNhanCQT.TDiep;
using DLieu303 = Services.ViewModels.XML.ThongDiepGuiNhanCQT.TDiepGuiHDonSaiSotMayTinhTien.DLieu;
using DLTBao303 = Services.ViewModels.XML.ThongDiepGuiNhanCQT.TDiepGuiHDonSaiSotMayTinhTien.DLTBao;
using DSCKS303 = Services.ViewModels.XML.ThongDiepGuiNhanCQT.TDiepGuiHDonSaiSotMayTinhTien.DSCKS;
using HDon303 = Services.ViewModels.XML.ThongDiepGuiNhanCQT.TDiepGuiHDonSaiSotMayTinhTien.HDon;
using TBao303 = Services.ViewModels.XML.ThongDiepGuiNhanCQT.TDiepGuiHDonSaiSotMayTinhTien.TBao;
using TDiep303 = Services.ViewModels.XML.ThongDiepGuiNhanCQT.TDiepGuiHDonSaiSotMayTinhTien.TDiep;
using TTChung303 = Services.ViewModels.XML.ThongDiepGuiNhanCQT.TDiepGuiHDonSaiSotMayTinhTien.TTChung;
...
namespace Services.Repositories.Implimentations.QuanLyHoaDon
{
    public class ThongDiepGuiNhanCQTService : IThongDiepGuiNhanCQTService
    {
        ...
        private readonly IQuanLyThongTinHoaDonService _IQuanLyThongTinHoaDonService;
        ...
        public ThongDiepGuiNhanCQTService(Datacontext db,
            ...
            IQuanLyThongTinHoaDonService quanLyThongTinHoaDonService,
        ...
        )
        {
            ...
            _ITuyChonService = tuyChonService;
            _IQuanLyThongTinHoaDonService = quanLyThongTinHoaDonService;
                        ...
                        select new ThongDiepGuiCQTViewModel
                        {
                            ...
                            ThongDiepChiTietGuiCQTs = (from chiTiet in queryDetail
                                                       ...
                                                       select new ThongDiepChiTietGuiCQTViewModel
                                                       {
                                                           ...
                                                           MaCQT12 = chiTiet.MaCQT12,
                                                           MaCQT45 = chiTiet.MaCQT45,
                                                           MaCQTToKhaiChapNhan = chiTiet.MaCQTToKhaiChapNhan,
                                                           MaCQTChuoiKyTuSo = chiTiet.MaCQTChuoiKyTuSo,
                                                           MauHoaDon = chiTiet.MauHoaDon,
                                                           MauSoHoaDon_LoaiHoaDon = chiTiet.MauSoHoaDon_LoaiHoaDon,
                                                           MauSoHoaDon_SoLienHoaDon = chiTiet.MauSoHoaDon_SoLienHoaDon,
                                                           MauSoHoaDon_SoThuTuMau = chiTiet.MauSoHoaDon_SoThuTuMau,
                                                           KyHieuHoaDon = chiTiet.KyHieuHoaDon,
                                                           KyHieu1 = chiTiet.KyHieu1,
                                                           KyHieu23 = chiTiet.KyHieu23,
                                                           KyHieu4 = chiTiet.KyHieu4,
                                                           KyHieu56 = chiTiet.KyHieu56,
                                                           KyHieuHoaDon_2KyTuDau = chiTiet.KyHieuHoaDon_2KyTuDau,
                                                           KyHieuHoaDon_2KyTuDau_DatIn = chiTiet.KyHieuHoaDon_2KyTuDau_DatIn,
                                                           KyHieuHoaDon_2SoCuoiNamThongBao = chiTiet.KyHieuHoaDon_2SoCuoiNamThongBao,
                                                           KyHieuHoaDon_HinhThucHoaDon = chiTiet.KyHieuHoaDon_HinhThucHoaDon,
                                                           ...
                                                           HinhThucHoaDon = chiTiet.HinhThucHoaDon,
                                                           LoaiHoaDon = (chiTiet.MauHoaDon == "9" || chiTiet.MauHoaDon == "10") ? chiTiet.MauHoaDon : null,
                                                       }
                                                      ).ToList()
                        };
        ...
        public async Task<List<HoaDonSaiSotViewModel>> GetListHoaDonSaiSotAsync(HoaDonSaiSotParams @params)
        {
            try
            {
                ...
                var tuyChons = await _ITuyChonService.GetAllAsync();
                var tuyChonGuiMTT = bool.Parse(tuyChons.Find(x => x.Ma == "BoolChoPhepXuLySaiSotMTTDuaTrenLichSuGuiEmail").GiaTri);
                ...
                if (string.IsNullOrWhiteSpace(@params.LapTuHoaDonDienTuId))
                {
                    ...
                    queryHoaDonDienTu = await _db.HoaDonDienTus.Include(x => x.BoKyHieuHoaDon).Where(x => x.BoKyHieuHoaDon != null && x.BoKyHieuHoaDon.HinhThucHoaDon == (HinhThucHoaDon)@params.HinhThucHoaDon
                ...
                if (@params.IsTBaoHuyGiaiTrinhKhacCuaNNT == true)
                {
                    ...
                    query = await (from hoadon in _db.ThongTinHoaDons
                                   ...
                                   && hoadon.TrangThaiGui04 != (int)TrangThaiGuiThongDiep.CoHoaDonCQTKhongTiepNhan
                                   )
                                   ...
                                   select new HoaDonSaiSotViewModel
                                   {
                                       ...
                                       LoaiHoaDon = (hoadon.LoaiHoaDon.ToString() == "9" || hoadon.LoaiHoaDon.ToString() == "10") ? hoadon.LoaiHoaDon.ToString() : null,
                                   }).ToListAsync();
                }
                else
                {
                    ...
                    var queryHoaDonDaGuiSaiSotCQTs = (from tdgcqt in _db.ThongDiepGuiCQTs
                                                      join tdgct in _db.ThongDiepChiTietGuiCQTs on tdgcqt.Id equals tdgct.ThongDiepGuiCQTId
                                                      join tdc in _db.ThongDiepChungs on tdgcqt.Id equals tdc.IdThamChieu
                                                      where tdc.TrangThaiGui != (int)TrangThaiGuiThongDiep.ChuaGui
                                                      select tdgct.HoaDonDienTuId).Distinct().ToList();
                    ...
                    var queryHoaDonHuyChuaGui = from hoadon in queryHoaDonDienTu
                                                ...
                                                || hoadon.TrangThaiGuiHoaDon < (int)TrangThaiGuiHoaDon.DaGui
                                                || hoadon.IsXacNhanDaGuiHDChoKhachHang != true
                                                || (@params.HinhThucHoaDon == (int)HinhThucHoaDon.CoMaTuMayTinhTien && tuyChonGuiMTT == false && hoadon.TrangThaiGuiHoaDonMTT < (int)TrangThaiGuiHoaDon.DaGui)
                                                || queryHoaDonDaGuiSaiSotCQTs.Contains(hoadon.HoaDonDienTuId))
                                                && bkhhd.HinhThucHoaDon == (HinhThucHoaDon)@params.HinhThucHoaDon
                                                && (DateTime.Parse(hoadon.NgayHoaDon.Value.ToString("yyyy-MM-dd")) >= fromDate || fromDate == null)
                                                ...
                                                select new HoaDonSaiSotViewModel
                                                {
                                                    ...
                                                    MaCQTCap = (bkhhd.HinhThucHoaDon == HinhThucHoaDon.CoMa || bkhhd.HinhThucHoaDon == HinhThucHoaDon.CoMaTuMayTinhTien) ? (hoadon.MaCuaCQT ?? "<Chưa cấp mã>") : "",
                                                    MauHoaDon = bkhhd.KyHieuMauSoHoaDon.ToString(),
                                                    KyHieuHoaDon = bkhhd.KyHieuHoaDon ?? "",
                                                    SoHoaDon = hoadon.SoHoaDon,
                                                    NgayLapHoaDon = hoadon.NgayHoaDon,
                    ...
                    var queryThamChieuHoaDonSaiThongTin = await (from hoadon in _db.NhatKyGuiEmails
                                                                 where (hoadon.LoaiEmail == LoaiEmail.ThongBaoSaiThongTinKhongPhaiLapLaiHoaDon || hoadon.LoaiEmail == LoaiEmail.ThongBaoSaiThongTinKhongPhaiLapLaiHoaDonHinhThucCoMaTuMTT || hoadon.LoaiEmail == LoaiEmail.ThongBaoSaiThongTinKhongPhaiLapLaiPXK) && !string.IsNullOrWhiteSpace(hoadon.So)
                    ...
                    var queryThamChieuHoaDonBiDieuChinh = from hoadon in queryHoaDonDienTu
                                                          ...
                                                          || (bkhhd.HinhThucHoaDon == HinhThucHoaDon.CoMaTuMayTinhTien && hoadon.TrangThaiQuyTrinh == (int)TrangThaiQuyTrinh.CQTDaCapMa)
                                                          || (bkhhd.HinhThucHoaDon == HinhThucHoaDon.CoMaTuMayTinhTien && hoadon.TrangThaiQuyTrinh == (int)TrangThaiQuyTrinh.HoaDonHopLe))
                    ...
                    queryHoaDonBiDieuChinh = queryHoaDonBiDieuChinh.Where(x => _db.ThongDiepChiTietGuiCQTs.Count(o => o.ChungTuLienQuan == x.ChungTuLienQuan) < 1 || (x.ChungTuLienQuan.Split(";").Length > 1 && _db.HoaDonDienTus.FirstOrDefault(y => x.ChungTuLienQuan.Contains(y.HoaDonDienTuId)).IsDaLapThongBao04 == false));
                ...
                }else if (@params.LoaiSaiSot == -1) //Lấy tất cả  trừ Thay thế và Điều chỉnh
                {
                    query = query.Where(x => x.PhanLoaiHDSaiSot != 3 && x.PhanLoaiHDSaiSot != 2).ToList();
                }
                ...
                else
                {
                    ...
                    if (listHoaDonCanDanhDau.Count > 0)
                    {
                        foreach (var item in listHoaDonCanDanhDau)
                        {
                            ...
                            var thongDiepChiTiet = children.FirstOrDefault(x => x.HoaDonDienTuId == item.HoaDonDienTuId);
                            if(!item.NgayGuiTBaoSaiSotKhongPhaiLapHD.HasValue && thongDiepChiTiet.ChungTuLienQuan.StartsWith("TBSS"))
                            {
                                var chungTuLienQuans = thongDiepChiTiet.ChungTuLienQuan.Split(";");
                                var chungTuLienQuan = chungTuLienQuans.Where(x => x.StartsWith("TBSS")).FirstOrDefault();

                                if (!string.IsNullOrEmpty(chungTuLienQuan))
                                {
                                    System.Globalization.CultureInfo enUS = new System.Globalization.CultureInfo("en-US");
                                    DateTime outTime;
                                    var suffix = chungTuLienQuan.Substring(chungTuLienQuan.LastIndexOf("-") + 1);
                                    DateTime.TryParseExact(suffix, "dd/MM/yyyy HH:mm:ss", enUS,
                                         System.Globalization.DateTimeStyles.None,
                                         out outTime);
                                    item.NgayGuiTBaoSaiSotKhongPhaiLapHD = outTime;
                                }
                            }
                        }
        ...
        /// <param name="model"></param>
        /// <returns>ThongDiepGuiCQTViewModel</returns>
        public async Task<KetQuaLuuThongDiep> InsertThongBaoGuiHoaDonSaiSot303Async(ThongDiepGuiCQTViewModel model)
        {
            ThongDiepChung thongDiepChung = null;
            if (string.IsNullOrWhiteSpace(model.Id) == false)
            {
            ...
            else
            {
                ...
                model.Id = Guid.NewGuid().ToString();
                model.MaThongDiep = "0200784873" + string.Join("", Guid.NewGuid().ToString().Split("-")).ToUpper();
                if (model.LoaiThongBao == 2)
                {
                    var entityTD = await _db.ThongDiepChungs.FirstOrDefaultAsync(x => x.IdThamChieu == model.ThongBaoHoaDonRaSoatId);
                    if (entityTD != null)
                    {
                        model.MaTDiepThamChieu = entityTD.MaThongDiep;
                    }
                }
            }
            ...
            if (ketQua > 0)
            {
                ...
                //đánh dấu hóa đơn đã lập thông báo 04
                var listIdHoaDonCanDanhDau = model.ThongDiepChiTietGuiCQTs.Select(x => x.HoaDonDienTuId).ToList();
                var lstHoaDonCanDanhDau = model.ThongDiepChiTietGuiCQTs;
                if (model.IsTBaoHuyGiaiTrinhKhacCuaNNT.GetValueOrDefault())
                {
                    //nếu là hóa đơn nhập từ phần mềm khác
                    var listHoaDonCanDanhDau = await _db.ThongTinHoaDons.Where(x => listIdHoaDonCanDanhDau.Contains(x.Id)).ToListAsync();
                    ...
                    var lstNam = lstHoaDonCanDanhDau.Select(x => x.KyHieu23).Distinct().ToList();
                    foreach(var it in lstNam)
                    {
                        var maxKyTuSo = lstHoaDonCanDanhDau.Where(x => x.KyHieu23 == it).Select(x => long.Parse(x.MaCQTChuoiKyTuSo)).Max();
                        await _IQuanLyThongTinHoaDonService.UpdateSoDaSinhMoiNhatHoaDonCMMTTien(maxKyTuSo);
                    }
                }
                else
                {
                ...
                var tDiepXML = await CreateXMLThongDiep303GuiCQT(fullFolder + "/" + tenFile + ".xml", model);
                ...
                //thêm bản ghi vào bảng thông điệp chung để hiển thị ra bảng kê
                string thongDiepChungId = await ThemDuLieu303VaoBangThongDiepChung(tDiepXML, ketQuaLuuDuLieu, thongDiepChung);
        ...
        private async Task<string> ThemDuLieu303VaoBangThongDiepChung(TDiep303 tDiep, KetQuaLuuThongDiep ketQuaLuuThongDiep, ThongDiepChung thongDiepChung)
        {
            ...
            ThongDiepChungViewModel model = new ThongDiepChungViewModel
            {
                ...
                MaThongDiepThamChieu = thongDiepChung != null ? (thongDiepChung.MaThongDiepThamChieu) : (!string.IsNullOrEmpty(tDiep.TTChung.MTDTChieu) ? tDiep.TTChung.MTDTChieu : DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss")),
                ThongDiepGuiDi = true,
            ...
            if (ketQua)
            {
                ...
                XmlSerializer serialiser = new XmlSerializer(typeof(TDiep303));
        ...
        private async Task<TDiep> CreateXMLThongDiepGuiCQT(string xmlFilePath, ThongDiepGuiCQTViewModel model)
        {
            try
            {
                ...
                {
                    PBan = "2.0.0",
                    MNGui = mNGui,       // "V0200784873", // "V0202029650",
                    MNNhan = mNNhan,     //"TVAN",
                    MLTDiep = (model.LoaiThongBao == 3 && model.ThongDiepChiTietGuiCQTs[0].HinhThucHoaDon == "2") ? 303 : MaLoaiThongDiep,
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private async Task<TDiep303> CreateXMLThongDiep303GuiCQT(string xmlFilePath, ThongDiepGuiCQTViewModel model)
        {
            try
            {
                string mNGui = _configuration["TTChung:MNGui"];
                string mNNhan = _configuration["TTChung:MNNhan"];
                TTChung303 ttChung = new TTChung303
                {
                    PBan = "2.0.0",
                    MNGui = mNGui,       // "V0200784873", // "V0202029650",
                    MNNhan = mNNhan,     //"TVAN",
                    MLTDiep = ((model.LoaiThongBao != 3 && model.ThongDiepChiTietGuiCQTs[0].LoaiHoaDon != null) || model.ThongDiepChiTietGuiCQTs[0].HinhThucHoaDon == "2") ? 303 : MaLoaiThongDiep,
                    MTDiep = model.MaThongDiep ?? "",
                    MTDTChieu = model.LoaiThongBao == 2 ? (model.MaTDiepThamChieu ?? "") : "", //đọc từ thông điệp nhận
                    MST = model.MaSoThue ?? "",
                    SLuong = 1
                ...
                List<HDon303> listHDon = new List<HDon303>();
                for (int i = 0; i < model.ThongDiepChiTietGuiCQTs.Count; i++)
                {
                    var item = model.ThongDiepChiTietGuiCQTs[i];
                    HDon303 hoaDon = new HDon303
                    {
                        STT = i + 1,
                        MCQTCap = item.MaCQTCap, //giá trị này ở bên hóa đơn điện tử
                        KHMSHDon = item.MauHoaDon ?? "",
                ...
                //Khắc phục lỗi của mcqt sai định dạng vì thuế nhận mã 5 ký tự
                var listCoQuanThue = new List<CoQuanThue>
                {
                    new CoQuanThue(){  MaCQTCapCuc="3101",Ma="10300",Ten="Cục Thuế Thành phố Hải phòng"},
                    new CoQuanThue(){  MaCQTCapCuc="3102",Ma="10301",Ten="Quận Hồng Bàng – Chi cục Thuế khu vực Hồng Bàng – An Dương"},
                    new CoQuanThue(){  MaCQTCapCuc="3103",Ma="10303",Ten="Quận Ngô Quyền – Chi cục Thuế khu vực Ngô Quyền – Hải An"},
                    new CoQuanThue(){  MaCQTCapCuc="3104",Ma ="10304",Ten="Quận Hải An – Chi cục Thuế khu vực Ngô Quyền – Hải An"},
                    new CoQuanThue(){  MaCQTCapCuc="3105",Ma ="10305",Ten="Quận Lê Chân – Chi cục Thuế khu vực Lê Chân – Dương Kinh"},
                    new CoQuanThue(){  MaCQTCapCuc="3106",Ma ="10307",Ten="Quận Kiến An – Chi cục Thuế khu vực Kiến An – An Lão"},
                    new CoQuanThue(){  MaCQTCapCuc="3107",Ma="10309",Ten="Quận Đồ Sơn – Chi cục Thuế khu vực Đồ Sơn – Kiến Thụy"},
                    new CoQuanThue(){  MaCQTCapCuc="3108",Ma="10311",Ten="Chi cục Thuế huyện Thủy Nguyên"},
                    new CoQuanThue(){  MaCQTCapCuc="3109",Ma="10313",Ten="Huyện An Dương – Chi cục Thuế khu vực Hồng Bàng – An Dương"},
                    new CoQuanThue(){  MaCQTCapCuc="3110",Ma="10315",Ten="Huyện An Lão – Chi cục Thuế khu vực Kiến An – An Lão"},
                    new CoQuanThue(){  MaCQTCapCuc="3111",Ma="10317",Ten="Huyện Kiến Thụy – Chi cục Thuế khu vực Đồ Sơn – Kiến Thụy"},
                    new CoQuanThue(){  MaCQTCapCuc="3112",Ma="10319",Ten="Huyện Tiên Lãng – Chi cục Thuế khu vực Vĩnh Bảo – Tiên Lãng"},
                    new CoQuanThue(){  MaCQTCapCuc="3113",Ma="10321",Ten="Huyện Vĩnh Bảo – Chi cục Thuế khu vực Vĩnh Bảo – Tiên Lãng"},
                    new CoQuanThue(){  MaCQTCapCuc="3114",Ma="10323",Ten="Chi cục Thuế huyện Cát Hải"},
                    new CoQuanThue(){  MaCQTCapCuc="10325",Ma ="10325",Ten="Chi cục Thuế huyện Bạch Long Vĩ"},
                    new CoQuanThue(){  MaCQTCapCuc="3115",Ma ="10327",Ten="Quận Dương Kinh – Chi cục Thuế khu vực Lê Chân – Dương Kinh"}
                };
                var mCQT = listCoQuanThue.FirstOrDefault(x => x.MaCQTCapCuc == model.MaCoQuanThue).Ma;
                DLTBao303 dLTBao = new DLTBao303
                {
                    PBan = "2.0.1",
                    MSo = "04/SS-HĐĐT",
                    Ten = "Thông báo hóa đơn điện tử có sai sót",
                ...
                DSCKS303 dSCKS = new DSCKS303
                {
                    NNT = ""
                };

                TBao303 tBao = new TBao303
                {
                    DLTBao = dLTBao,
                    DSCKS = dSCKS
                };

                DLieu303 DLieu = new DLieu303
                {
                    TBao = tBao
                };

                TDiep303 tDiep = new TDiep303
                {
                    TTChung = ttChung,
                    DLieu = DLieu
                };
        ...
        public async Task<string> GateForWebSocket(FileXMLThongDiepGuiParams @params)
        {
            try
            {
                ...
                var entityBangThongDiepChungToUpdate = await _db.ThongDiepChungs.FirstOrDefaultAsync(x => x.IdThamChieu == @params.ThongDiepGuiCQTId && x.MaLoaiThongDiep == @params.MaLoaiThongDiep && x.TrangThaiGui == (int)TrangThaiGuiThongDiep.ChuaGui);
                ...
                var thongDiepChiTiets = await _db.ThongDiepChiTietGuiCQTs.Where(x => x.ThongDiepGuiCQTId == @params.ThongDiepGuiCQTId).ToListAsync();
                if (thongDiepChiTiets.Count > 0)
                {
                    var listIdHoaDonCanDanhDau = thongDiepChiTiets.Select(x => x.HoaDonDienTuId).ToList();
                    ...
                    else
                    {
                        ...
                        if (listHoaDonCanDanhDau.Count > 0)
                        {
                            foreach (var item in listHoaDonCanDanhDau)
                            {
                                var thongDiepChiTiet = thongDiepChiTiets.FirstOrDefault(x => x.HoaDonDienTuId == item.HoaDonDienTuId);
                                item.IsDaLapThongBao04 = true;
                                item.LanGui04 = await _db.ThongDiepChiTietGuiCQTs.Where(x=>x.HoaDonDienTuId == item.HoaDonDienTuId && x.PhanLoaiHDSaiSot == thongDiepChiTiet.PhanLoaiHDSaiSot).CountAsync();
                                item.TrangThaiGui04 = (int)TrangThaiGuiThongDiep.ChoPhanHoi;
                                item.PhanLoaiSaiSot04 = thongDiepChiTiet.PhanLoaiHDSaiSot;
                                item.ChungTuLienQuan = thongDiepChiTiet.ChungTuLienQuan;
                                item.IsUpdated04 = true;
                            }
                            _db.HoaDonDienTus.UpdateRange(listHoaDonCanDanhDau);
                            await _db.SaveChangesAsync();
                        }
        ...
        public async Task<bool> GuiThongDiepToiCQTAsync(DuLieuXMLGuiCQTParams @params)
        {
            try
            {
                ...
                var responce999 = string.Empty;
                if (@params.MaLoaiThongDiep == (int)MLTDiep.TDTBHDDTKTTMTTCSSot)
                {
                    responce999 = await _ITVanService.TVANSendData("api/error-invoice/send303", xmlContent);
                }
                else responce999 = await _ITVanService.TVANSendData("api/error-invoice/send", xmlContent);
                ...
                {
                    var ttChung = Helper.XmlHelper.GetTTChungFromStringXML(responce999);
                    if (ttChung.MLTDiep == ((int)MLTDiep.TDPHSDDang).ToString())
                    {
                        guiTCTNLoi = true;
                    }
                    else
                ...
                var entityBangThongDiepChungToUpdate = await _db.ThongDiepChungs.FirstOrDefaultAsync(x => x.IdThamChieu == @params.ThongDiepGuiCQTId && x.MaLoaiThongDiep == @params.MaLoaiThongDiep && x.TrangThaiGui == (int)TrangThaiGuiThongDiep.ChoPhanHoi);
                if (entityBangThongDiepChungToUpdate != null)
                {
                ...
                if (listIdHoaDonCanDanhDau.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (listHoaDonCanDanhDau.Count > 0)
                        {
                            foreach (var item in listHoaDonCanDanhDau)
                            {
                                if (item.TrangThaiGui04 != (int)TrangThaiGuiThongDiep.CQTTiepNhanTatCaHoaDon && item.TrangThaiGui04 != (int)TrangThaiGuiThongDiep.GoiDuLieuHopLe)
                                {
                                    if (guiTCTNLoi)
                                    {
                                        item.TrangThaiGui04 = (int)TrangThaiGuiThongDiep.GuiTCTNLoi;
        ...
        public async Task<List<ThongBaoHoaDonRaSoatViewModel>> GetListHoaDonRaSoatAsync(HoaDonRaSoatParams @params)
        {
            ...
            var query = from hoaDon in _db.ThongBaoHoaDonRaSoats
                        join bkh in _db.ThongBaoChiTietHoaDonRaSoats on hoaDon.Id equals bkh.ThongBaoHoaDonRaSoatId
                        where
                        (
                        ...
                        (
                            ...
                            && (@params.HinhThucHoaDon == -1 || (@params.HinhThucHoaDon == (int)HinhThucHoaDon.CoMa ? (bkh.KyHieuHoaDon.StartsWith("C") && bkh.KyHieuHoaDon.Substring(3,1) != "M")
                            : @params.HinhThucHoaDon == (int)HinhThucHoaDon.CoMaTuMayTinhTien ? (bkh.KyHieuHoaDon.StartsWith("C") && bkh.KyHieuHoaDon.Substring(3, 1) == "M")
                            : bkh.KyHieuHoaDon.StartsWith("K")))
                        ...
                        select new ThongBaoHoaDonRaSoatViewModel
                        {
                            ...
                            Lan = hoaDon.Lan,
                            TinhTrang = hoaDon.NgayThongBao.AddHours(hoaDon.ThoiHan) > DateTime.Now,
        ...
        public async Task<List<ThongBaoChiTietHoaDonRaSoatViewModel>> GetListChiTietHoaDonRaSoatAsync(string thongBaoHoaDonRaSoatId)
        {
            var query = from hoaDon in _db.ThongBaoChiTietHoaDonRaSoats
                        ...
                        select new ThongBaoChiTietHoaDonRaSoatViewModel
                        {
                            ...
                            LyDo = hoaDon.LyDo == null ? hoaDon.LyDoRaSoat : hoaDon.LyDo
                        };
        ...
        public async Task<string> ThemThongBaoHoaDonRaSoat(ViewModels.XML.ThongDiepGuiNhanCQT.TDiepNhanHDonRaSoat.TDiep tDiep)
        {
            ...
            if (ketQua)
            {
                ...
                {
                    var ct = new ThongBaoChiTietHoaDonRaSoat
                    {
                    ...
                    ct.MaCQTCap = await (from hd in _db.HoaDonDienTus
                                   join bkh in _db.BoKyHieuHoaDons on hd.BoKyHieuHoaDonId equals bkh.BoKyHieuHoaDonId
                                   where hd.SoHoaDon == long.Parse(item.SHDon) && bkh.KyHieuMauSoHoaDon == int.Parse(item.KHMSHDon)
                                   && bkh.KyHieuHoaDon == item.KHHDon && hd.NgayHoaDon == DateTime.Parse(item.Ngay)
                                   select hd.MaCuaCQT).FirstOrDefaultAsync();

                    var tDiepGoc = await _db.ThongDiepChungs.FirstOrDefaultAsync(x => x.MaThongDiep == tDiep.TTChung.MTDTChieu);
                    if(tDiepGoc == null || (tDiepGoc.MaLoaiThongDiep != (int)MLTDiep.TDTBHDDLSSot && tDiepGoc.MaLoaiThongDiep != (int)MLTDiep.TDTBHDDTKTTMTTCSSot))
                    {
                        ct.LyDo = item.LDo;
                    }
                    else
                    {
                        var thongDiepGuiCQT = await _db.ThongDiepGuiCQTs.FirstOrDefaultAsync(x => x.Id == tDiepGoc.IdThamChieu);
                        var thongDiepChiTiet = await _db.ThongDiepChiTietGuiCQTs.FirstOrDefaultAsync(x => x.ThongDiepGuiCQTId == thongDiepGuiCQT.Id && (x.KyHieuHoaDon == $"{item.KHMSHDon}{item.KHHDon}" || x.KyHieuHoaDon == item.KHHDon) && x.SoHoaDon == item.SHDon && x.NgayLapHoaDon == DateTime.Parse(item.Ngay));
                        ct.LyDo = thongDiepChiTiet.LyDo;
                    }
        ...
        public async Task<List<BangKeHoaDonSaiSot_ViewModel>> GetBangKeHoaDonSaiSotAsync(ThongKeHoaDonSaiSotParams thongKeHoaDonSaiSotParams)
        {
            var queryBangKe = await (from hoaDon in _db.ThongDiepChiTietGuiCQTs
                                     ...
                                     select new BangKeHoaDonSaiSot_ViewModel
                                     {
                                         ...
                                         MauSoTBaoPhanHoiTuCQT = thongDiepChung.MaLoaiThongDiep == 301 ? "01/TB-HĐSS" : "",
            ...
            else
            {
                ...
                else if (phanLoaiBanGhi == "emailThongBaoSaiThongTin")
                {
                    ...
                    if (thongBaoSaiThongTin != null)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(thongBaoSaiThongTin.CanCuocCongDan_Dung))
                        {
                            if (thongBaoSaiThongTin.CanCuocCongDan_Dung.TrimToUpper() != thongBaoSaiThongTin.CanCuocCongDan_Sai.TrimToUpper())
                            {
                                if (string.IsNullOrWhiteSpace(thongBaoSaiSot))
                                {
                                    thongBaoSaiSot = "Căn cước công dân đúng là: " + thongBaoSaiThongTin.CanCuocCongDan_Dung;
                                }
                                else
                                {
                                    thongBaoSaiSot += "; Căn cước công dân đúng là: " + thongBaoSaiThongTin.CanCuocCongDan_Dung;
                        ...
                        if (!string.IsNullOrWhiteSpace(thongBaoSaiThongTin.SoDienThoai_Dung))
                        {
                            if (thongBaoSaiThongTin.SoDienThoai_Dung.TrimToUpper() != thongBaoSaiThongTin.SoDienThoai_Sai.TrimToUpper())
                            {
                                if (string.IsNullOrWhiteSpace(thongBaoSaiSot))
                                {
                                    thongBaoSaiSot = "Số điện thoại đúng là: " + thongBaoSaiThongTin.SoDienThoai_Dung;
                                }
                                else
                                {
                                    thongBaoSaiSot += "; Số điện thoại đúng là: " + thongBaoSaiThongTin.SoDienThoai_Dung;
                        ...
                        if (!string.IsNullOrWhiteSpace(thongBaoSaiThongTin.ThongTinGiaiTrinhSaiSot))
                        {
                            if (string.IsNullOrWhiteSpace(thongBaoSaiSot))
                            {
                                thongBaoSaiSot = thongBaoSaiThongTin.ThongTinGiaiTrinhSaiSot;
                            }
                            else
                            {
                                thongBaoSaiSot += "; " + thongBaoSaiThongTin.ThongTinGiaiTrinhSaiSot;
        ...
        private ThongTinChuKySoViewModel GetThongTinNguoiKy_202(XmlDocument xmlDoc, string loaiChuKy)
        {
            try
            {
                //loaiChuKy: là các thẻ chữ ký số trong thẻ <DSCKS>. Ở trong file này: CQT

                var signature = xmlDoc.SelectNodes("/TDiep/DLieu/HDon/DSCKS/" + loaiChuKy);
                if (signature != null)
                {
                    if (signature.Count > 0)
                    {
        ...
        /// <summary>
        /// Lấy ngày cơ quan thuế cấp mã 202
        /// </summary>
        /// <param name="hoaDonDienTuId"></param>
        /// <returns></returns>
        public async Task<string>GetNgayCoQuanThueCapMa202(string hoaDonDienTuId)
        {
            try
            {
                var thongDiepChung200 = await (from hoaDon in _db.HoaDonDienTus
                                               join dulieuhd in _db.DuLieuGuiHDDTs on hoaDon.HoaDonDienTuId equals dulieuhd.HoaDonDienTuId
                                               join thongDiepChung in _db.ThongDiepChungs on dulieuhd.DuLieuGuiHDDTId equals thongDiepChung.IdThamChieu
                                               where hoaDon.HoaDonDienTuId == hoaDonDienTuId
                                               select thongDiepChung).FirstOrDefaultAsync();

                var thongDiep202 = await _db.ThongDiepChungs.FirstOrDefaultAsync(x => x.MaLoaiThongDiep == 202 && x.MaThongDiepThamChieu == thongDiepChung200.MaThongDiep);
                if (thongDiep202 != null)
                {
                    var xmlContent202 = await _db.FileDatas.Where(x => x.RefId == thongDiep202.ThongDiepChungId && x.Type == 1).Select(x => x.Content).FirstOrDefaultAsync();
                    XmlDocument xmlDoc = new XmlDocument();
                    xmlDoc.LoadXml(xmlContent202);
                    var chuKyCua_CQT = GetThongTinNguoiKy_202(xmlDoc, "CQT");
                    return chuKyCua_CQT.NgayKy;
                }
            }
            catch(Exception ex)
            {
        ...
        private bool IsTBaoHuyKhongDuocChapNhan(HoaDonDienTu hoaDon, List<ThongDiepGuiCQT> thongDiepGuiCQTs, List<ThongDiepChiTietGuiCQT> tdcts, List<ThongDiepChung> thongDiepChungs)
        {
            try
            {
                if (hoaDon == null) return false;
                if (hoaDon.TrangThai != (int)TrangThaiHoaDon.HoaDonXoaBo) return false;
                var query = (from tdgcqt in thongDiepGuiCQTs
                             join tdctcqt in tdcts on tdgcqt.Id equals tdctcqt.ThongDiepGuiCQTId
                             join tdc in thongDiepChungs on tdgcqt.Id equals tdc.IdThamChieu
                             where tdctcqt.HoaDonDienTuId == hoaDon.HoaDonDienTuId && tdctcqt.PhanLoaiHDSaiSot == 1 && tdc.TrangThaiGui > (int)TrangThaiGuiThongDiep.ChuaGui
                             orderby tdc.NgayGui descending
                             select tdc).FirstOrDefault();

                if (query == null) return false;
                return query.TrangThaiGui == (int)TrangThaiGuiThongDiep.GuiTCTNLoi
                      || query.TrangThaiGui == (int)TrangThaiGuiThongDiep.GuiLoi
                      || query.TrangThaiGui == (int)TrangThaiGuiThongDiep.GoiDuLieuKhongHopLe
                      || query.TrangThaiGui == (int)TrangThaiGuiThongDiep.CoHoaDonCQTKhongTiepNhan
                      || query.TrangThaiGui == (int)TrangThaiGuiThongDiep.CoHDKhongHopLe;
            }
            catch (Exception ex)
            {
