...
namespace Services.Repositories.Implimentations.DanhMuc
{
    public class DoiTuongService : IDoiTuongService
    {
        ...
        /// <summary>
        /// Check Trung Version 3
        /// </summary>
        /// <param name="model"></param>
        /// <returns></returns>
        public async Task<DoiTuong> CheckTrungMaPosAsync(DoiTuongViewModel model)
        {

            DoiTuong result = new DoiTuong();
            try
            {
                result = await _db.DoiTuongs.Where(x => x.Ma != null && x.Ma.ToUpper().Trim() == model.Ma.ToUpper().Trim()).FirstOrDefaultAsync();
                if (result == null)
                {
                    return null;
                }
            ...
            {
                Tracert.WriteLog("CheckTrungMaPosAsync:: " + ex.Message);
            }
            return result;
        }
        /// <summary>
        ...
        /// <param name="ma"></param>
        /// <returns></returns>
        public async Task<DoiTuong> GetKhachHangByMa(string ma)
        {

            DoiTuong result = new DoiTuong();
            try
            {
                result = await _db.DoiTuongs.Where(x => x.IsKhachHang == true && x.Ma != null && x.Ma.ToUpper().Trim() == ma.ToUpper().Trim()).AsNoTracking().FirstOrDefaultAsync();
                if (result == null)
                {
                    return null;
                }
            ...
            {
                Tracert.WriteLog("GetKhachHangByMa:: " + ex.Message);
            }
            return result;
        }
        ...
        /// <param name="ma"></param>
        /// <returns></returns>
        public async Task<DoiTuong> GetNhanVienByMa(string ma)
        {

            DoiTuong result = new DoiTuong();
            try
            {
                result = await _db.DoiTuongs.Where(x =>x.IsNhanVien ==true && x.Ma != null && x.Ma.ToUpper().Trim() == ma.ToUpper().Trim()).FirstOrDefaultAsync();
                if (result == null)
                {
                    return null;
                }
            ...
            {
                Tracert.WriteLog("GetNhanVienByMa:: " + ex.Message);
            }
            return result;
        }
        ...
        public async Task<PagedList<DoiTuongViewModel>> GetAllPagingAsync(DoiTuongParams @params)
        {
            ...
            if (@params.TimKiemTheo != null)
            {
                ...
                if (!string.IsNullOrEmpty(timKiemTheo.Ma))
                {
                    var keyword = timKiemTheo.Ma.ToUpper().ToTrim();
                    query = query.Where(x => x.Ma.ToUpper().ToTrim().Contains(keyword));
                }
                if (!string.IsNullOrEmpty(timKiemTheo.Ten))
                {
                    var keyword = timKiemTheo.Ten.ToUpper().ToTrim();
                    query = query.Where(x => x.Ten.ToUpper().ToTrim().Contains(keyword));
                }
                ...
                if (!string.IsNullOrEmpty(timKiemTheo.NguoiMuaHang))
                {
                    ...
                    query = query.Where(x => x.HoTenNguoiMuaHang.ToTrim().Contains(keyword));
                }
            }
        ...
        {
            DoiTuongViewModel result = new DoiTuongViewModel();
            try
            {
                var entity = _mp.Map<DoiTuong>(model);
                await _db.DoiTuongs.AddAsync(entity);
                await _db.SaveChangesAsync();
                result = _mp.Map<DoiTuongViewModel>(entity);
            }
            catch (Exception ex)
            {
                Tracert.WriteLog("Bug Add DoiTuong " + ex.Message);
            }
            return result;
        }
        public async Task<ResultParams> FilterInsertKhachHangAsync(DoiTuongViewModel model)
        {
            ...
            try
            {
                ...
                if (model.LoaiKhachHang != 1 && model.LoaiKhachHang != 2)
                {
                    result.ErrorMessage = "LoaiKhachHang sai định dạng";
                    result.Success = false;
                    return result;
                }
                if (model.IsKhachHang == false || model.IsNhanVien == true)
                {
                    result.ErrorMessage = "IsKhachHang phải là khách hàng";
                    result.Success = false;
                    return result;
                }
                //check
                ...
                {
                    if (DataValidator.CheckValidMaSoThue(model.MaSoThue) == false)
                    {
                        msg += "MaSoThue không hợp lệ. MST hợp lệ là: 1-Để trống; 2-Số ký tự của MST bằng 10 hoặc bằng 14";
                        kq = false;
                    }
                }
                if (string.IsNullOrEmpty(model.Ma))
                {
                    msg += string.Format("Ma không được để trống;");
                    kq = false;
                }
                else
                {
                    if (model.Ma.Length > 50)
                    {
                        msg += string.Format("Ma không vượt quá 50 ký tự;");
                        kq = false;
                    }
                    else
                    {
                        var dt = await _db.DoiTuongs.Where(x =>x.DoiTuongId != model.DoiTuongId && x.Ma != null && x.Ma.ToUpper().Trim() == model.Ma.ToUpper().Trim()).FirstOrDefaultAsync();
                        if (dt != null)
                        {
                            msg += string.Format("Ma đã tồn tại trong hệ thống;");
                            kq = false;
                        }
                    }
                }
                ...
                {
                    msg += string.Format("Ten không được để trống;");
                    kq = false;
                }
                else if (model.Ten.Length > 400)
                {
                        msg += string.Format("Ten không vượt quá 400 ký tự;");
                        kq = false;
                }
                if (!string.IsNullOrEmpty(model.DiaChi) && model.DiaChi.Length > 400)
                {
                        msg += string.Format("DiaChi không vượt quá 400 ký tự;");
                        kq = false;
                }
                if (!string.IsNullOrEmpty(model.SoTaiKhoanNganHang) && model.SoTaiKhoanNganHang.Length > 30)
                {
                        msg += string.Format("SoTaiKhoanNganHang không vượt quá 30 ký tự;");
                        kq = false;
                }
                if (model.SoTaiKhoanNganHang != "" && model.TenNganHang == "")
                {
                    msg += "TenNganHang không được để trống vì đã nhập SoTaiKhoanNganHang";
                    kq = false;
                }
                if (model.SoTaiKhoanNganHang == "" && model.TenNganHang != "")
                {
                    msg += "SoTaiKhoanNganHang không được để trống vì đã nhập TenNganHang";
                    kq = false;
                }
                if (!string.IsNullOrEmpty(model.TenNganHang) && model.TenNganHang.Length > 400)
                {
                    msg += string.Format("TenNganHang không vượt quá 400 ký tự;");
                    kq = false;
                }
                if (model.ChiNhanh != "" && model.SoTaiKhoanNganHang == "" && model.TenNganHang == "")
                {
                    msg += "ChiNhanh không được nhập vì SoTaiKhoanNganHang và TenNganHang đang để trống";
                    kq = false;
                }
                if (!string.IsNullOrEmpty(model.CanCuocCongDan) && model.CanCuocCongDan.Length > 12)
                {
                        msg += string.Format("CanCuocCongDan không vượt quá 12 ký tự;");
                        kq = false;
                }
                if (!string.IsNullOrEmpty(model.HoTenNguoiMuaHang) && model.HoTenNguoiMuaHang.Length > 100)
                {
                        msg += string.Format("HoTenNguoiMuaHang không vượt quá 100 ký tự;");
                        kq = false;
                }
                if (!string.IsNullOrEmpty(model.EmailNguoiMuaHang))
                {
                    if (model.EmailNguoiMuaHang.Length > 50)
                    {
                        msg += string.Format("EmailNguoiMuaHang không vượt quá 50 ký tự;");
                        kq = false;
                    }
                    else
                    {
                        if (!DataValidator.CheckValidEmail(model.EmailNguoiMuaHang))
                        {
                            msg += string.Format("EmailNguoiMuaHang sai định dạng email;");
                            kq = false;
                        }
                    }
                }
                if (!string.IsNullOrEmpty(model.SoDienThoaiNguoiMuaHang))
                {
                    if (model.SoDienThoaiNguoiMuaHang.Length > 20)
                    {
                        msg += string.Format("SoDienThoaiNguoiMuaHang không vượt quá 20 ký tự;");
                        kq = false;
                    }
                    else
                    {
                        if (!DataValidator.CheckValidPhone(model.SoDienThoaiNguoiMuaHang))
                        {
                            msg += string.Format("SoDienThoaiNguoiMuaHang sai định dạng số điện thoại;");
                            kq = false;
                        }
                    }
                }
                if (!string.IsNullOrEmpty(model.EmailNguoiNhanHD))
                {
                    if (model.EmailNguoiNhanHD.Length > 50)
                    {
                        msg += string.Format("EmailNguoiNhanHD không vượt quá 50 ký tự;");
                        kq = false;
                    }
                    else
                    {
                        if (!DataValidator.CheckValidEmail(model.EmailNguoiNhanHD))
                        {
                            msg += string.Format("EmailNguoiNhanHD không hợp lệ;");
                            kq = false;
                        }
                    }
                }
                if (!string.IsNullOrEmpty(model.SoDienThoaiNguoiNhanHD))
                {
                    if (model.SoDienThoaiNguoiNhanHD.Length > 20)
                    {
                        msg += string.Format("SoDienThoaiNguoiNhanHD không vượt quá 20 ký tự;");
                        kq = false;
                    }
                    else
                    {
                        if (!DataValidator.CheckValidPhone(model.SoDienThoaiNguoiNhanHD))
                        {
                            msg += string.Format("SoDienThoaiNguoiNhanHD không hợp lệ;");
                            kq = false;
                        }
                    }
                }
            ...
            {
                result.Success = false;
                result.ErrorMessage = "Lỗi thêm mới: " + ex.Message;
                Tracert.WriteLog("Bug DoiTuong FilterInsertAsync: " + ex.Message);
            }
            return result;
        }
        ...
        public async Task<List<DoiTuongViewModel>> ImportKhachHang(IList<IFormFile> files, int modeValue)
        {
            ...
            //var listNV = await _db.DoiTuongs.Where(x => x.IsKhachHang == true).ToListAsync();
        ...
        public async Task<List<DoiTuongViewModel>> GetAllPagingKhachHangXuatKhauAsync(DoiTuongParams @params)
        {
            var query = _db.DoiTuongs.Where(x => x.IsKhachHang == true)
                .OrderBy(x => x.Ma)
                .Select(x => new DoiTuongViewModel
                {
                    DoiTuongId = x.DoiTuongId,
            ...
            if (!string.IsNullOrEmpty(@params.Keyword)) //trường hợp tìm kiếm mã và tên trên combobox
            {
                query = query.Where(x => x.Ma.Trim().ToUpper().Contains(@params.Keyword.Trim().ToUpper()) ||
                                       x.Ten.Trim().ToUpper().Contains(@params.Keyword.Trim().ToUpper()) ||
                                       x.MaSoThue.Trim().ToUpper().Contains(@params.Keyword.Trim().ToUpper()) ||
                                       x.HoTenNguoiMuaHang.Trim().ToUpper().Contains(@params.Keyword.Trim().ToUpper()));
            }
