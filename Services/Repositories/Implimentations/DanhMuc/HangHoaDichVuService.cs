...
namespace Services.Repositories.Implimentations.DanhMuc
{
    public class HangHoaDichVuService : IHangHoaDichVuService
    {
        ...
        public HangHoaDichVuService(Datacontext datacontext, IMapper mapper, IHostingEnvironment hostingEnvironment, IHttpContextAccessor httpContextAccessor, IHoSoHDDTService hoSoHDDTService, IDonViTinhService donViTinhService)
        {
            _db = datacontext;
            _mp = mapper;
        ...
        /// <summary>
        /// GetHangHoaDichVuByMa
        /// </summary>
        /// <param name="ma"></param>
        /// <returns></returns>
        public async Task<HangHoaDichVuViewModel> GetHangHoaDichVuByMa(string ma)
        {

            var result = new HangHoaDichVuViewModel();
            try
            {
                var query = from hhdv in _db.HangHoaDichVus
                            join dvt in _db.DonViTinhs on hhdv.DonViTinhId equals dvt.DonViTinhId into tmpDonViTinhs
                            from dvt in tmpDonViTinhs.DefaultIfEmpty()
                            where hhdv.Ma.ToUpper().Trim() == ma.ToUpper().Trim()
                            select new HangHoaDichVuViewModel
                            {
                                HangHoaDichVuId = hhdv.HangHoaDichVuId,
                                Ma = hhdv.Ma,
            ...
            {
                Tracert.WriteLog("bug Check Trung " + ex.Message);
            }
            return result;
        }
        public async Task<ResultParams> FilterPosSyncGoods(HangHoaDichVuViewModel item)
        {
            ...
            try
            {
                ...
                if (item.Ma == "")
                {
                    msg += "<Mã hàng> không được để trống";
                    kq = false;
                }
                else if (item.Ma.Length > 50)
                {
                    msg += "<Mã hàng> không vượt quá 50 ký tự.";
                    kq = false;
                }
                else if (await CheckTrungMaAsync(item))
                {
                    var dt = await _db.HangHoaDichVus.Where(x => x.HangHoaDichVuId != item.HangHoaDichVuId && x.Ma != null && x.Ma.ToUpper().Trim() == item.Ma.ToUpper().Trim()).FirstOrDefaultAsync();
                    if (dt != null)
                    {
                        msg += string.Format("<Mã hàng> đã tồn tại");
                        kq = false;
                    }
                }
                ...
                {
                    msg += "<Tên hàng> không được để trống";
                    kq = false;
                }
                else
                {
                    ...
                    {
                        msg += "<Tên hàng> không vượt quá 400 ký tự.";
                        kq = false;
                    }
                }

                if (!string.IsNullOrEmpty(item.TenDonViTinh) && item.TenDonViTinh.Length > 50)
                {
                    msg += "<Đơn vị tính> không vượt quá 50 ký tự";
                    kq = false;
                }
                #endregion
                ...
                {
                    msg += "<ThueGTGT> không được để trống";
                    kq = false;
                }
                if (!string.IsNullOrEmpty(item.ThueGTGT) && item.ThueGTGT != "0" && item.ThueGTGT != "5" && item.ThueGTGT != "10" && item.ThueGTGT != "KCT" && item.ThueGTGT != "KKKNT" && item.ThueGTGT != "KHAC")
                {
                    msg += "<Thuế GTGT> không hợp lệ. Danh mục ThueGTGT cho phép: {0;5;10;KCT;KKKNT;KHAC}";
                    kq = false;
                }
            ...
            catch (Exception ex)
            {
                ...
                Tracert.WriteLog("Function FilterPosSyncGoods: " + ex.Message);
            }
            return result;
        }
