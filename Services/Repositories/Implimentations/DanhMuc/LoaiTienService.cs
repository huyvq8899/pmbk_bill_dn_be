...
namespace Services.Repositories.Implimentations.DanhMuc
{
    public class LoaiTienService : ILoaiTienService
    {
        ...
        public async Task<LoaiTienViewModel> GetByMaAsync(string maLoaiTien)
        {
            var entity = await _db.LoaiTiens.AsNoTracking().FirstOrDefaultAsync(x => x.Ma == maLoaiTien && x.Status==true);
            var result = _mp.Map<LoaiTienViewModel>(entity);
            return result;
        }
