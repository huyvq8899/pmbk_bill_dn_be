using DLL.Entity.DanhMuc;
using Services.Helper;
using Services.ViewModels.DanhMuc;
using System;
using System.Collections.Generic;

namespace Services.ViewModels.Params
{
    public class BangKeHoaDonDieuChinh
    {
        public string HoaDonBiDieuChinhId { get; set; }
        public string MaTraCuuBiDieuChinh { get; set; }
        public int? LoaiHoaDonBiDieuChinh { get; set; }
        public string TenLoaiHoaDonBiDieuChinh { get; set; }
        public DateTime? NgayHoaDonBiDieuChinh { get; set; }
        public string SoHoaDonBiDieuChinh { get; set; }
        public string MauSoBiDieuChinh { get; set; }
        public string KyHieuBiDieuChinh { get; set; }

        public string HoaDonDieuChinhId { get; set; }
        public int? TrangThaiHoaDonDieuChinh { get; set; }
        public string TenTrangThaiHoaDonDieuChinh { get; set; }
        public string TenHinhThucHoaDonBiDieuChinh { get; set; }
        public int? LoaiDieuChinh { get; set; }
        public string TenLoaiDieuChinh { get; set; }
        public string LyDoDieuChinh { get; set; }
        public int? TrangThaiBienBanDieuChinh { get; set; }
        public string TenTrangThaiBienBanDieuChinh { get; set; }
        public string MaTraCuuDieuChinh { get; set; }
        public int? LoaiHoaDonDieuChinh { get; set; }
        public string TenLoaiHoaDonDieuChinh { get; set; }
        public DateTime? NgayHoaDonDieuChinh { get; set; }
        public string SoHoaDonDieuChinh { get; set; }
        public string MauSoDieuChinh { get; set; }
        public string KyHieuDieuChinh { get; set; }
        public string MaKhachHangDieuChinh { get; set; }
        public string TenKhachHangDieuChinh { get; set; }
        public string MaSoThueDieuChinh { get; set; }
        public string NguoiMuaHangDieuChinh { get; set; }
        public string NhanVienBanHangDieuChinh { get; set; }
        public string LoaiTienId { get; set; }
        public string MaLoaiTien { get; set; }
        public bool? IsVND { get; set; }
        public decimal? TongTienThanhToan { get; set; }

        public string BienBanDieuChinhId { get; set; }

        public List<TaiLieuDinhKemViewModel> TaiLieuDinhKems { get; set; }
    }
}
