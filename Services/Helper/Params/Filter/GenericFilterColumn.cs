...
namespace Services.Helper.Params.Filter
{
    ...
    public class GenericFilterColumn<T> where T : class
    {
        ...
        public static List<T> Query(List<T> query, Func<T, object> selector, FilterColumn filterColumn, FilterValueType filterValueType)
        {
            if (filterValueType != FilterValueType.DateTime) filterColumn.ColValue = (filterColumn.ColValue ?? string.Empty).ToUnSign().ToUpper().Trim();
            ...
            {
                case FilterCondition.Chua:
                    query = query.Where(x => (selector(x) ?? string.Empty).ToString().ToUnSign().ToUpper().Contains(filterColumn.ColValue)).ToList();
                    break;
                case FilterCondition.KhongChua:
                    query = query.Where(x => !(selector(x) ?? string.Empty).ToString().ToUnSign().ToUpper().Contains(filterColumn.ColValue)).ToList();
                    break;
                case FilterCondition.Bang:
                    ...
                    {
                        case FilterValueType.Decimal:
                            query = query.Where(x => selector(x) != null && (((decimal)selector(x)) == decimal.Parse(filterColumn.ColValue))).ToList();
                            break;
                        case FilterValueType.String:
                            query = query.Where(x => (selector(x) ?? string.Empty).ToString().ToUnSign().ToUpper().Equals(filterColumn.ColValue)).ToList();
                            break;
                        case FilterValueType.DateTime:
                            var colValueDate = DateTime.Parse(filterColumn.ColValue);
                            query = query.Where(x => selector(x) != null && DateTime.Parse(selector(x).ToString()) == colValueDate).ToList();
                            break;
                        default:
                            break;
                    }
                ...
                case FilterCondition.Khac:
                    ...
                    {
                        case FilterValueType.Decimal:
                            query = query.Where(x => !(selector(x) ?? string.Empty).Equals(filterColumn.ColValue)).ToList();
                            break;
                        case FilterValueType.String:
                            query = query.Where(x => !(selector(x) ?? string.Empty).ToString().ToUnSign().ToUpper().Equals(filterColumn.ColValue)).ToList();
                            break;
                    ...
                    break;
                case FilterCondition.BatDau:
                    query = query.Where(x => (selector(x) ?? string.Empty).ToString().ToUnSign().ToUpper().StartsWith(filterColumn.ColValue)).ToList();
                    break;
                case FilterCondition.KetThuc:
                    query = query.Where(x => (selector(x) ?? string.Empty).ToString().ToUnSign().ToUpper().EndsWith(filterColumn.ColValue)).ToList();
                    break;
                case FilterCondition.NhoHon:
                    switch (filterValueType)
                    {
                ...
                case FilterCondition.LonHonHoacBang:
                    ...
                    break;
                case FilterCondition.Trong:
                    query = query.Where(x => selector(x) == null || string.IsNullOrEmpty(selector(x).ToString())).ToList();
                    break;
                case FilterCondition.KhongTrong:
                    query = query.Where(x => selector(x) != null && !string.IsNullOrEmpty(selector(x).ToString())).ToList();
                    break;
                case FilterCondition.TuNgayDenNgay:
                    ...
                    var toDate = DateTime.Parse(filterColumn.ColValue.Split('|')[1]);
                    query = query.Where(x => selector(x) != null && DateTime.Parse(selector(x).ToString()) >= fromDate &&
                    DateTime.Parse(selector(x).ToString()) <= toDate).ToList();
                    break;
                default:
                    break;
            }
