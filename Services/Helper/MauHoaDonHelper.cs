    ...
    public static class MauHoaDonHelper
    {
        ...
        public static Document TaoMauHoaDonDoc(MauHoaDonViewModel mauHoaDon, HinhThucMauHoaDon loai, IHostingEnvironment env, IHttpContextAccessor accessor, out int beginRow, bool hasReason = false)
        {
            lock (obj)
            {
                ...
                string docPath = "";
                if (mauHoaDon.TenBoMau == "01.CB.05")
                {
                    docPath = Path.Combine(webRootPath, $"docs/MauHoaDon/01.CB.05.docx");
                }
                else
                {
                    docPath = Path.Combine(webRootPath, $"docs/MauHoaDon/01.CB.01.docx");
                }
        ...
        private static void StyleDataTable(this Document doc, Table table, List<MauHoaDonTuyChinhChiTietViewModel> list, TableType tableType, MauHoaDonViewModel mauHoaDon, HinhThucMauHoaDon loai)
        {
            ...
            if (table != null)
            {
                if (tableType == TableType.ThongTinNguoiBan)
                {
                    ...
                    if (logo != null && mauHoaDon.TenBoMau != "01.CB.05")
                    {
                    ...
                    if (hasLogo)
                    {
                        ...
                        picLogo.VerticalPosition = topLogo + (100 / heightLogo * 3) + (listThongTinNguoBan.Count * (listThongTinNguoBan.Count * 30 / 130f));
                        picLogo.HorizontalPosition = leftLogo;
                        picLogo.Width = widthLogo;
                        picLogo.Height = heightLogo;
                        picLogo.TextWrappingStyle = TextWrappingStyle.Tight;
                    }
                    ...
                    /// Mẫu CB05 - Style lại border của khung viền - thông tin người bán
                    if (mauHoaDon.TenBoMau == "01.CB.05")
                    {
                ...
                if (tableType == TableType.ThongTinHoaDon)
                {
                    ...
                    // table.ResetCells(row, col);
                    TableRow cl_row_default = table.Rows[0].Clone(); // Cột mặc định để add mã cqt
                    if (hasLogo)
                    {
                        if (positionLogo == 1)
                        {
                            table.ApplyVerticalMerge(0, 0, 2); // Merger column Logo
                        }
                        else
                        {
                            table.ApplyVerticalMerge(col - 1, 0, row - 2);
                        }
                    }
                    ////////////////////
                    for (int i = 0; i < table.Rows.Count - 1; i++)
                    {
                        ...
                        if (hasLogo)
                        {
                            table.Rows[i].Cells[0].Width = widthLogo * (maxCol == 2 ? 90 : 60) / 100;
                        }
                        else
                        {
                            widthOfTieuDe += 10;
                    ...
                    if (hasLogo)
                    {
                        ...
                        if (positionLogo == 1)
                        {
                            // table.ApplyVerticalMerge(0, 0, row - 2);
                            paraLogo = table[0, 0].AddParagraph();
                        }
                        else
                        {
                        ...
                        Image logoImage = Image.FromFile(logoPath);
                        DocPicture picLogo = paraLogo.AppendPicture(logoImage);
                        picLogo.VerticalPosition = topLogo + (100 / heightLogo * 3) + (listThongTinHoaDon.Count * (listThongTinHoaDon.Count * 50 / 100f));
                        picLogo.HorizontalPosition = leftLogo;
                        picLogo.Width = widthLogo;
                        picLogo.Height = heightLogo;
                    ...
                    if (mauHoaDon.HinhThucHoaDon == HinhThucHoaDon.CoMa || mauHoaDon.HinhThucHoaDon == HinhThucHoaDon.CoMaTuMayTinhTien)
                    {
                        ...
                        if (maCuaCQT != null)
                        {
                            if (mauHoaDon.TenBoMau == "01.CB.05")
                            {
                                table.Rows.Insert(rowTieuDe, cl_row_default);
                    ...
                    /// Mẫu CB05 - Style lại border của khung viền - thông tin hóa đơn
                    if (mauHoaDon.TenBoMau == "01.CB.05")
                    {
                        table.TableFormat.Borders.BorderType = BorderStyle.Cleared;
                        table.TableFormat.Borders.Bottom.BorderType = BorderStyle.Single;
                ...
                if (tableType == TableType.ThongTinNguoiMua)
                {
                    ...
                    if (mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonGTGT || mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonBanHang || mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonGTGTCMTMTT || mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonBanHangCMTMTT || mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonKhacCMTMTT || mauHoaDon.LoaiHoaDon == LoaiHoaDon.TemVeBanHang)
                    {
                        listThongTinChung = cloneList.Where(x => x.Loai == LoaiTuyChinhChiTiet.ThongTinNguoiMua && ((x.LoaiChiTiet >= LoaiChiTietTuyChonNoiDung.HoTenNguoiMua && x.LoaiChiTiet <= LoaiChiTietTuyChonNoiDung.DiaChiNguoiMua) || x.LoaiChiTiet == LoaiChiTietTuyChonNoiDung.SoDienThoaiNguoiMua || x.LoaiChiTiet == LoaiChiTietTuyChonNoiDung.CanCuocCongDanNguoiMua || x.LoaiChiTiet == LoaiChiTietTuyChonNoiDung.TenPhuongTien)).ToList();
                ...
                if (tableType == TableType.ThongTinHangHoaDichVu)
                {
                    ...
                    if (mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonGTGT || mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonBanHang || mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonGTGTCMTMTT || mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonBanHangCMTMTT || mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonKhacCMTMTT || mauHoaDon.LoaiHoaDon == LoaiHoaDon.TemVeBanHang)
                    {
                        if (mauHoaDon.LoaiThueGTGT == LoaiThueGTGT.MauMotThueSuat || mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonBanHangCMTMTT || mauHoaDon.LoaiHoaDon == LoaiHoaDon.TemVeBanHang)
                        {
                            ...
                            for (int i = 0; i < amountRow; i++)
                            {
                                ...
                                else
                                {
                                    ...
                                    if ((mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonBanHang || mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonBanHangCMTMTT || mauHoaDon.LoaiHoaDon == LoaiHoaDon.TemVeBanHang) && (itemLeft.LoaiChiTiet == LoaiChiTietTuyChonNoiDung.TongTienThanhToan ||
                                ...
                                if (itemLeft != null)
                                {
                                                                                            ...
                                                                                            itemLeft.LoaiChiTiet == LoaiChiTietTuyChonNoiDung.CongTienHang))
                                    {
                                        ...
                                        foreach (var child in tieuDeItemLefts)
                                        {
                                            if (child.LoaiContainer == LoaiContainerTuyChinh.TieuDe)
                                            {
                                                if ((mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonBanHang || mauHoaDon.LoaiHoaDon == LoaiHoaDon.HoaDonBanHangCMTMTT || mauHoaDon.LoaiHoaDon == LoaiHoaDon.TemVeBanHang) && child.LoaiChiTiet == LoaiChiTietTuyChonNoiDung.CongTienHang)
                                                {
        ...
        public static FileReturn PreviewFilePDF(MauHoaDonViewModel mauHoaDon, HinhThucMauHoaDon loai, HoSoHDDTViewModel hoSoHDDT, IHostingEnvironment env, IHttpContextAccessor accessor)
        {
            ...
            if(mauHoaDon.LoaiHoaDon == LoaiHoaDon.TemVeGTGT)
            {
                y = (page.Canvas.ClientSize.Height - 100) / 2;
            }
