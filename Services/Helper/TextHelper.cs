...
namespace ManagementServices.Helper
{
    public static class TextHelper
    {
        ...
        public static string FormatNumberByTuyChon(this decimal value, List<TuyChonViewModel> tuyChons, string loai, bool showZerro = false, string maLoaiTien = null)
        {
            ...
            var ttt = tuyChons.Where(x => x.Ma == "IntKyTuNganCach").Select(x => x.GiaTri).FirstOrDefault();
            var kyTuNganCach = ttt != null ? int.Parse(ttt) : 0;
        ...
        public static string ConvertToInWord(this decimal total, string cachDocSo0HangChuc, string cachDocSoHangNghin, bool hienThiSoChan, string maLoaiTien, int cachTheHienSoTienBangChu, HoaDonDienTuViewModel hd)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(maLoaiTien) && (hd.TrangThai == (int)TrangThaiHoaDon.HoaDonDieuChinh) && ((hd.LoaiDieuChinh == (int)LoaiDieuChinhHoaDon.DieuChinhTang) || (hd.LoaiDieuChinh == (int)LoaiDieuChinhHoaDon.DieuChinhGiam || (hd.LoaiDieuChinh == (int)LoaiDieuChinhHoaDon.DieuChinhChung))))
                {
                    switch (cachTheHienSoTienBangChu)
                    {
                        case 1: // [Giảm/Tăng] [Tổng tiền thanh toán]
                            rs = (hd.LoaiDieuChinh == 1 ? "Tăng" : hd.LoaiDieuChinh == 2 ? "Giảm" : (hd.LoaiDieuChinh == 0 && totalDecimal > 0) ? "Tăng" : (hd.LoaiDieuChinh == 0 && totalDecimal < 0) ? "Giảm" : "") + " " + rs.ToLower();
                            break;
                        case 2: // [Điều chỉnh giảm/Điều chỉnh tăng] [Tổng tiền thanh toán]
                            rs = (hd.LoaiDieuChinh == 1 ? "Điều chỉnh tăng" : hd.LoaiDieuChinh == 2 ? "Điều chỉnh giảm" : (hd.LoaiDieuChinh == 0 && totalDecimal > 0) ? "Điều chỉnh tăng" : (hd.LoaiDieuChinh == 0 && totalDecimal < 0) ? "Điều chỉnh giảm" : "") + " " + rs.ToLower();
                            break;
                        case 3: // [Tổng tiền thanh toán] [(Giảm)/(Tăng)]
                            rs = rs + " " + (hd.LoaiDieuChinh == 1 ? "(Tăng)" : hd.LoaiDieuChinh == 2 ? "(Giảm)" : (hd.LoaiDieuChinh == 0 && totalDecimal > 0) ? "(Tăng)" : (hd.LoaiDieuChinh == 0 && totalDecimal < 0) ? "(Giảm)" : "");
                            break;
                        case 4: // [Tổng tiền thanh toán] [(Điều chỉnh giản)/(Điều chỉnh tăng)]
                            rs = rs + " " + (hd.LoaiDieuChinh == 1 ? "(Điều chỉnh tăng)" : hd.LoaiDieuChinh == 2 ? "(Điều chỉnh giảm)" : (hd.LoaiDieuChinh == 0 && totalDecimal > 0) ? "(Điều chỉnh tăng)" : (hd.LoaiDieuChinh == 0 && totalDecimal < 0) ? "(Điều chỉnh giảm)" : "");
                            break;
                        case 5: // [Âm/] [Tổng tiền thanh toán]
                            if (hd.LoaiDieuChinh == 2 || (totalDecimal < 0 && hd.LoaiDieuChinh == 0)) // điều chỉnh giảm
                            {
                                rs = $"Âm {rs.ToLower()}";
                            }
                            break;
                ...
                return rs + (!string.IsNullOrEmpty(maLoaiTien) ? "." : "");
            }
            catch
            {
                return "";
        ...
        public static string GetThueHasPer(this string value)
        {
            ...
            if (value == "KCT" || value == "KKKNT" || value == "KHAC")
            {
                return value;
            }
            if(value == "KHAC:0")
        ...
        public static string GetEmailWithType(string value, int type) // 1: email chính, 2 CC, 3 BCC
        {
            ...
            foreach (var item in array)
            {
                switch (type)
                {
                    ...
                    case 2:
                        ...
                        {
                            result.Add(item.Substring(4));
                        }
                        break;
                    case 3:
                        if (item.StartsWith("(BCC)"))
                        {
                            result.Add(item.Substring(5));
                        }
                        break;
                    default:
                        break;
        ...
        public static string GetCRC(string info)
        {
        ...
        public static string GetTenLoaiHoaDonFull(this LoaiHoaDon loaiHoaDon)
        {
        ...
        public static string TrimSpaceFristAndLastString(string input)
        {
            if (input == null || input == " ")
            {
                return "";
        ...
        public static string GenerateMaTraCuu()
        {
        ...
        public static string GetTenLoaiHoaDon(int type)
        {
            var kq = string.Empty;
            if (type == 1)
            {
                kq = "Hóa đơn giá trị gia tăng";
            }
            else if (type == 2)
            {
                kq = "Hóa đơn bán hàng";
            ...
            {
                kq = "Hóa đơn bán hàng (dành cho tổ chức, cá nhân trong khu phi thuế quan)";
            }
            else if (type == 4)
            {
                kq = "PXK kiêm vận chuyển nội bộ";
            }
            else
            {
                kq = "PXK hàng gửi bán đại lý";
            }
            return kq;
        }
