...
namespace API.Controllers.DanhMuc
{
    public class MauHoaDonController : BaseController
    {
        ...
        [HttpGet("GetBorderA5s")]
        public IActionResult GetBorderA5s()
        {
            var result = _mauHoaDonService.GetBorderA5s();
            return Ok(result);
        }
        ...
        /// <summary>
        /// API check Mau Hoa Don in Database
        /// </summary>
        /// <param name="mauHoaDonId"></param>
        /// <returns></returns>
        [HttpGet("CheckExist/{mauHoaDonId}")]
        public async Task<IActionResult> CheckExist(string mauHoaDonId)
        {
            var result = await _mauHoaDonService.CheckExistAsync(mauHoaDonId);
