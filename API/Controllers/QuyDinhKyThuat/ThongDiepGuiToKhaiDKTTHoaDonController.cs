...
namespace API.Controllers.QuyDinhKyThuat
{
    public class ThongDiepGuiToKhaiDKTTHoaDonController : BaseController
    {
        ...
        /// </summary>
        /// <returns></returns>
        [HttpGet("IsHasToKhaiDuocChapNhan")]
        public async Task<IActionResult> IsHasToKhaiDuocChapNhan()
        {
            var result = await _IToKhaiService.IsHasToKhaiDuocChapNhan();
            return Ok(result);
        }
        ...
        public IActionResult GetXMLToKhai(ToKhaiParams @params)
        {
            ...
            if (@params.ToKhaiKhongUyNhiem != null)
            {
                ...
                var fullXmlFolder = Path.Combine(_hostingEnvironment.WebRootPath, result);
                XDocument xdoc = XDocument.Load(fullXmlFolder);
                xdoc.Element("TKhai").Element("DLTKhai").Elements("NDTKhai").Elements("HTHDon").Elements("CMTMTTien")
                     .Where(x => x.Value == "0")
                     .Remove();
                xdoc.Save(fullXmlFolder);
            }
            else
            {
                if (!string.IsNullOrEmpty(@params.ToKhaiId))
        ...
        public async Task<IActionResult> GetAllPagingThongDiepChung(ThongDiepChungParams pagingParams)
        {
            ...
            /// Cập nhật lấy thêm mã giao dịch điện tử từ bảng TransferLogs
            var paged = await _IQuyDinhKyThuatService.GetPagingThongDiepChungInTransLogsAsync(pagingParams);
            if (paged != null)
            {
                foreach (var item in paged.Items)
                {
                    ...
                    if (item.ThongDiepGuiDi == false && item.MaLoaiThongDiep == (int)MLTDiep.TDTBHDDTCRSoat)
                    {
                        ...
                        var tDiepPhanHoi = _db.ThongDiepChungs.Where(x => (x.MaLoaiThongDiep == (int)MLTDiep.TDTBHDDLSSot || x.MaLoaiThongDiep == (int)MLTDiep.TDTBHDDTKTTMTTCSSot) && x.MaThongDiepThamChieu == item.MaThongDiep).FirstOrDefault();
		...
		/// <summary>
        /// Get Trạng thái gửi CQT của TB sai sót
        /// </summary>
        /// <param name="thongDiepId"></param>
        /// <returns></returns>
        [HttpGet("GetTrangThaiGuiToKhaiDangKyThayDoi/{thongDiepId}")]
        public async Task<IActionResult> GetTrangThaiGuiToKhaiDangKyThayDoi(string thongDiepId)
        {
            var result = await _IQuyDinhKyThuatService.GetTrangThaiGuiToKhaiDangKyThayDoi(thongDiepId);
            return Ok(result);
        }
        ...
        [HttpPost("GetThongDiep103FromFile")]
        public async Task<IActionResult> GetThongDiep103FromFile([FromForm] NhapKhauParams formData)
        {
            var result = await _IToKhaiService.GetThongDiep103FromFile(formData.Files[0]);
            return Ok(result);
        }
        ...
        /// <returns></returns>
        [HttpPost("XacNhanToKhai01")]
        public async Task<IActionResult> XacNhanToKhai01([FromForm] XacNhanToKhaiParams formData)
        {
            var result = await _IToKhaiService.XacNhanToKhai01(formData.ThongDiepId, formData.MaSoThue, formData.Files[0]);
            return Ok(result);
        }

        /// <summary>
        /// Kiểm tra trạng thái chữ ký số 
        /// </summary>
        /// <param name="formData"></param>
        /// <returns></returns>
        [HttpPost("VerifyFile103")]
        public async Task<IActionResult> VerifyFile103([FromForm] NhapKhauParams formData)
        {
            var result = _IToKhaiService.VerifyFile103(formData.Files[0]);
            return Ok(result);
        }

        /// <summary>
        /// Check mã số thuế và ngày ký thông điệp 103
        /// </summary>
        /// <param name="formData"></param>
        /// <returns></returns>
        [HttpPost("CheckThongDiep103")]
        public async Task<IActionResult> CheckThongDiep103([FromForm] NhapKhauParams formData)
        {
            var result = await _IToKhaiService.CheckThongDiep103(formData.Files[0]);
            return Ok(result);
        }
        /// <summary>
        /// Update file import 103
        /// </summary>
        /// <param name="files"></param>
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        [HttpGet("GetFile103Imported")]
        public async Task<IActionResult> GetFile103Imported(string FileId)
        {
            var result = await _IToKhaiService.GetFile103Imported(FileId);
        ...
        [HttpPost("UpdateFile103AndMCQTToHoSo")]
        public async Task<IActionResult> UpdateFile103AndMCQTToHoSo(IList<IFormFile> files, string MaCQT)
        {
            var result = await _IToKhaiService.UpdateFile103AndMCQTToHoSo(files, MaCQT);
            if (result == "") throw new Exception("");
            return Ok(new
            {
                result
        ...
        public class ParamUpdateHoSoImportMCQt {
